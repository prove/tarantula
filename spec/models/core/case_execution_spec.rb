...
describe CaseExecution do
  ...
  it "#update_with_steps should update" do
    ...
    ce.should_receive(:update_result).once
    se = flexmock('step execution')
    se.should_receive(:update_attributes!).once.with(
      {:result => Passed, :bug => {:id => 1}, :comment => 'com'})
    flexmock(StepExecution).should_receive(:find).once.and_return(se)

    ce.update_with_steps!({'duration' => 10},
      [{'id' => 1, 'result' => 'PASSED', 'bug' => {:id => 1}, 'comment' => 'com'}],
      @user)
  end
